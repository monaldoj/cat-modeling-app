    ...
    try:
        ...
        query = f"""
                    ...
                    SELECT
                        ...
                        AVG({catastrophe_type}) as count
                        -- AVG(coalesce({catastrophe_type}, 0)) as count
...
def create_legend(septiles):
    ...
    legend_items = [
        {"color": "#FED976", "label": f"< {septiles[1]:.3f}"},
        {"color": "#FEB24C", "label": f"{septiles[1]:.3f}-{septiles[2]:.3f}"},
        {"color": "#FD8D3C", "label": f"{septiles[2]:.3f}-{septiles[3]:.3f}"},
        {"color": "#FC4E2A", "label": f"{septiles[3]:.3f}-{septiles[4]:.3f}"},
        {"color": "#E31A1C", "label": f"{septiles[4]:.3f}-{septiles[5]:.3f}"},
        {"color": "#BD0026", "label": f"{septiles[5]:.3f}-{septiles[6]:.3f}"},
        {"color": "#800026", "label": f"≥ {septiles[6]:.3f}"}
    ]
...
def create_linear_color_scale(data, n_colors=7):
...
def create_leaflet_map(map_data, zoom=None, center=None, bounds=None, wkt=None):
...
def update_map_and_legend(n_clicks, center, zoom, bounds, catastrophe_type, event_name):
    ...
    if n_clicks is None:
        ...
        return leaflet_map, legend, active_style
    else:
        ...
        if event_name:
            ...
            flyto_bounds = wkt_to_bounds(event_wkt[0])
        ...
        return new_leaflet_map, new_legend, active_style
...
# Callback to populate catalog dropdown on app load
@app.callback(
    [Output('map-container', 'bounds')],
    [Input('event-dropdown', 'value')],
    prevent_initial_call=False
)
def populate_events(event_name):
    ...
    if event_name:
        ...
        global_bounds = flyto_bounds
        return {"bounds": global_bounds, "transition": {"duration": 1000}}
